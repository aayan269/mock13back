...
app.post("/login",async(req,res)=>{
  ...
  if(user){
    if(user.password==password || await argon2.verify(user.password,password)){
        const token=jwt.sign({id:user._id,username:user.username,email:user.email,role:user.role},"SECRET",{expiresIn:"24 hours"})
        const refreshToken=jwt.sign({id:user._id,username:user.username,email:user.email,role:user.role},"REFRESH",{expiresIn:"7 days"})
